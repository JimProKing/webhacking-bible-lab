...
### 2.6 요청 가로채기(Intercept)와 하이재킹

사이트 메인 페이지 하단에 **"6. 요청 가로채기 & 하이재킹"** 섹션이 추가되어 있다.

주요 포인트:
- **Network 탭에서 "가로채기"**: 폼을 그냥 클릭 → Network에서 요청 우클릭 → **Copy as fetch** (Edit and Resend가 안 보일 때 이걸 쓰세요).
- 붙여넣은 fetch 코드를 Console에서 바로 수정해서 재전송 (user 파라미터 바꾸기, 쿠키 강제 주입 등).
- **Set-Cookie 훔치기**: 로그인 응답의 Response Headers에서 `Set-Cookie` 값을 찾는 법.
- **수동 하이재킹**: 훔친 쿠키 값을 Console이나 curl로 넣어서 /myinfo에 접근하기.
- 이 단계가 끝나면 "이제 진짜 모든 트래픽을 자동으로 가로채고 싶다"는 욕구가 생길 거예요. 그때 Burp를 배우면 됩니다.

자세한 초보자용 스텝은 사이트 메인 페이지의 "6. 요청 가로채기 & 하이재킹" 섹션에 아주 자세히 적어놨습니다.
